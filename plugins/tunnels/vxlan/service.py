"""
IronShield Plugin — vxlan
Path: plugins/tunnels/vxlan/service.py
Purpose: vxlan tunnel implementation.
"""
...
logger = get_service_logger("vxlan")

SYSTEMD_SERVICE = "ironshield-vxlan"
BIN_PATH = Path("/usr/local/bin/vxlan")


class VxlanService(BaseService):
    """vxlan tunnel plugin."""
    ...
    def uninstall(self) -> Result:
        ...
        return Result.ok("vxlan uninstalled")

    def start(self) -> Result:
        ok = systemctl("start", SYSTEMD_SERVICE)
        return Result.ok("vxlan started") if ok else Result.fail("Failed to start vxlan")

    def stop(self) -> Result:
        systemctl("stop", SYSTEMD_SERVICE)
        return Result.ok("vxlan stopped")
